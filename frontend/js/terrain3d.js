 ...
 * Renders only in the sidebar panel (no central viewport).
 */
...
function initTerrain3D() {
  ...
  const parent = document.getElementById('terrain3d-container');
  if (!container || !parent) return;

  if (typeof OrbitControls !== 'function') {
    console.error('[3D Terrain] OrbitControls not loaded (import failed or wrong Three.js version)');
    throw new Error('OrbitControls is not a constructor. Try a hard refresh (Ctrl+Shift+R).');
  }
  ...
  const gridHelper = new THREE.GridHelper(200, 50, 0x1a2530, 0x151d26);
...
function createTerrainMesh(elevationData, metadata) {
  ...
  const minElev = metadata.elevation_stats.min_m;
  const maxElev = metadata.elevation_stats.max_m;
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      ...
      const normalizedHeight = ((elevation - minElev) / elevRange) * heightExaggeration * 10;
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      ...
      if (t < 0.2) {
        ...
        const t2 = (t - 0.5) / 0.3;
        r = 0.29 + t2 * 0.25; g = 0.49 - t2 * 0.1; b = 0.23 + t2 * 0.1;
      } else {
        const t2 = (t - 0.8) / 0.2;
        r = 0.55 + t2 * 0.08; g = 0.45 + t2 * 0.05; b = 0.35 + t2 * 0.03;
      }
...
async function loadTerrainData() {
  ...
  const resetBtn = document.getElementById('reset-terrain-view-btn');

  if (loading) loading.style.display = 'flex';

  try {
    const regionsRes = await fetch(`${API_BASE}/api/terrain/regions`);
    ...
    const response = await fetch(`${API_BASE}/api/terrain`, {
      method: 'POST',
    ...
    const isOrbitControls = /OrbitControls|constructor/i.test(err.message);
    const hint = isOrbitControls
...
function onDomReady() {
  const loadBtn = document.getElementById('load-terrain-btn');
  const resetBtn = document.getElementById('reset-terrain-view-btn');
